...
struct s_dhwt {
	struct {
		...
		bool electric_hasthermostat;	///< true if electric self-heating has its own thermostat. When switching to electric, self-heating relay will be permanently on (even if overtemp); and if none of the DHWT sensors are working the electric self-heating relay will be turned on in all active modes. *Defaults to false*
		...
		inid_t tid_bottom;		///< temp sensor id at bottom of dhw tank. *Optional* if #tid_top is set, *Required* otherwise
		inid_t tid_top;			///< temp sensor id at top of dhw tank. *Optional* if #tid_bottom is set, *Required* otherwise
		inid_t tid_win;			///< temp sensor id heatwater inlet. *Required* if #p.pump_feed is set. @note must @b NOT rely on pump_feed operation for accurate temp read.
		inid_t sid_selfheatershed;	///< switch sensor id, stops electric charge (if any) when active (load shedding). *Optional*
		outid_t rid_selfheater;		///< relay for internal electric heater (if available). *Optional*
